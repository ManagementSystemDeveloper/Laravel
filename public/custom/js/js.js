var js = {
    ...
    initializeGoogle:function() {
      ...
      if(input){
        ...
        google.maps.event.addListener(autocomplete, 'place_changed', function () {
            ...
            js.updateNearbyRestaurants(input.value);

            IsplaceChange = true;
        });
      }
      var input1 = document.getElementById('txtlocation1');
      if(input1) {
        ...
        google.maps.event.addListener(autocomplete, 'place_changed', function () {
            ...
            js.updateNearbyRestaurants(input1.value);
    ...
    updateNearbyRestaurants: function(address) {
      console.log(address);
    ...
    setGeolocation: function(data) {
      $.ajax({
          ...
          success: function(result) {
            var text_length_limit = 30;
            var address_text = data.address.slice(0, text_length_limit) + (data.address.length > text_length_limit ? "..." : "");
            $('#top_address_info').html(address_text);
            $('#txtlocation').html(data.address);
            $('#txtlocation1').html(data.address);
            if(typeof loadNearbyRestaurants != 'undefined') {
              setTimeout(() => {
